...
public class MatrixClass {
	...
	public void VariableType ()
	{
		...
		for (int i = (this.Brs - 1); i >= 0; i--)
		{
			...
			else if (!this.IsBaris0(i))
			{
				...
				// Jika kanan kosong atau semua nilai di kanannya memiliki nilai pasti
				// maka elemen ini memiliki nilai pasti
		...
		// Output apakah ada solusi atau tidak
		if (isSolusi)
		{
	...
	public void Parametrik()
	{
		...
		// Mendaftar semua relasi terhadap 
		...
		for (int i = (this.Brs - 1); i >= 0; i--)
		{
			// Memproses baris yang tidak 0, jika baris 0 langsung naik ke baris yang lebih atas
			if (!IsBaris0(i))
			{
				int markj = Marker(i); // nilai marker dari sebuah matriks
				if (vartype[markj] == 1) // nilai pasti
				{
					// Hitung nilai
					...
					// Menambah ke array solusi
					this.solusi[markj] = String.format("%.2f", nilai[this.Kol - 1][markj]);
				}
				else if (vartype[markj] == 2) // nilai terikat
				{
					// Hitung nilai
					nilai[this.Kol - 1][markj] = this.Mat[i][this.Kol - 1];
					for (int j = markj + 1; j <= (this.Kol - 2); j++)
					...
					// Menulis solusi ke array solusi
					String s = new String();
					boolean first = true;
					s = "";
					
					for (int k = 0; k <= (this.Kol - 2); k++)
					{
						
						if (!first && nilai[k][markj] > 0)
						{
							s += "+";
						}
						...
						if (nilai[k][markj] != 0)
						{
							...
							s += String.format("%.2f", nilai[k][markj]);
							char asconvert = (char)(k + 97);
							s += asconvert;
						}
					}
					if (nilai[this.Kol - 1][markj] > 0 && !first)
					{
						s += "+";
					}
					s += String.format("%.2f", nilai[this.Kol - 1][markj]);
					solusi[markj] = s;
				}
				else if (vartype[markj] == 3) // nilai bebas
				{
					// Tambah variabel saja
					char asconvert = (char)(markj + 97);
					this.solusi[markj] = "" + asconvert;
				}
			}
		}
	}
